...
const float NORMALIZATION_FACTOR = 20 * log10((float) (1<<ElectrosenseContext::getInstance()->getLog2FftSize()));
  ...
  while (mRunning) {
    ...
    if (mQueueIn && mQueueIn->try_dequeue(segment)) {
      ...
      for (uint32_t t = 0; t < signal_len / 2; t = t + 1) {
        ...
        power[t] = static_cast<signed char>(round(MAX(10.0f * log10(p) - NORMALIZATION_FACTOR, -100)));
