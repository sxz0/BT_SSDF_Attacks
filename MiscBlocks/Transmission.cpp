...
void Transmission::run() {
  ...
  while (mRunning || mQueueIn->size_approx() != 0) {
    ...
    if (mQueueIn && mQueueIn->try_dequeue(segment)) {
     ...
     //------------------------------------------------------------------------------------------------
     // Save segment samples in a file
     //---------------------------------
...
//      mFileName = "/root/es-sensor/buffer";
//      mOutputFile.open(mFileName, std::fstream::app);

//      mOutputFile << "Time seconds: " << segment->getTimeStamp().tv_sec << " Nanoseconds: " << segment->getTimeStamp().tv_nsec << "  Center_Freq: " << segment->getCenterFrequency() << std::endl;
//      mOutputFile << "PSD Values: "  << std::endl;

//      std::vector<float> psdsamples = segment->getPSDValues();

//      for (unsigned int i = 0; i < psdsamples.size(); i++)
//        mOutputFile <<  psdsamples.at(i) << ",";
